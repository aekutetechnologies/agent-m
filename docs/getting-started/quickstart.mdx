...
Put your key in `~/.agent-m/agent/auth.json` (created automatically on first
...
Key resolution order: `~/.agent-m/agent/auth.json` (nested
`{"providers": {"deepseek": {"apiKey": "..."}}}` or flat
`{"deepseek": "..."}`) → `~/.agent-m/agent/settings.json` (same shapes — but
that file is world-readable, so prefer `auth.json`). Environment variables
such as `DEEPSEEK_API_KEY` are **not** consulted.
...
- `/mode plan` switches to read-only plan mode; `/mode build` returns.
- `ctrl+d` exits (EOF). `ctrl+c` also exits the REPL.
...
| `preferences.json` | learned preferences — the "Known preferences" prompt block, fed by `/undo` usage |
| `settings.json` | model, providers, API keys |
